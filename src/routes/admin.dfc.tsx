    ...
    <AdminLayout>
      ...
      <div className="px-8 lg:px-12 pb-12 grid gap-8">
        ...
        <div className="dm-card">
          <div className="dm-cap mb-1">Gráfico</div>
          <div className="dm-serif text-[22px] mb-7">
        ...
        <div className="dm-card p-0 overflow-hidden">
          <div className="px-6 py-4" style={{ borderBottom: "1px solid var(--line)" }}>
            <div className="dm-cap mb-1">Detalhamento</div>
            <div className="dm-serif text-[22px]">Por <em className="italic" style={{ color: "var(--green)" }}>categoria</em></div>
          </div>
          <table className="w-full">
            <thead>
              <tr style={{ background: "var(--linen)" }}>
                {["Categoria", "Total", "% do total", "vs mês anterior"].map((h) => (
                  <th key={h} className="text-left px-6 py-3 dm-cap" style={{ fontWeight: 500 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {porCategoria.map((row, i) => {
                ...
                return (
                  <tr key={row.cat} style={{ background: i % 2 === 0 ? "#fff" : "var(--linen2)", borderTop: "1px solid var(--line)" }}>
                    ...
                    <td className="px-6 py-3 text-[12px] dm-serif" style={{ fontSize: 14, color: "var(--navy)" }}>{brl(row.val)}</td>
        ...
        <div className="dm-card p-0 overflow-hidden">
          <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: "1px solid var(--line)" }}>
            <div>
              <div className="dm-cap mb-1">Próximos 90 dias</div>
              <div className="dm-serif text-[22px]">Projeção <em className="italic" style={{ color: "var(--green)" }}>baseada em recorrências</em></div>
            </div>
              ...
              {projecao.map((p, i) => {
                ...
                return (
                  <tr key={p.mes} style={{ background: i % 2 === 0 ? "#fff" : "var(--linen2)", borderTop: "1px solid var(--line)" }}>
                    ...
                    <td className="px-6 py-3 text-[12px] dm-serif" style={{ fontSize: 14, color: "var(--green)" }}>{brl(p.rec)}</td>
                    <td className="px-6 py-3 text-[12px] dm-serif" style={{ fontSize: 14, color: "var(--tan)" }}>{brl(p.des)}</td>
                    <td className="px-6 py-3 text-[12px] dm-serif" style={{ fontSize: 16, color: r >= 0 ? "var(--green)" : "var(--tan)" }}>{brl(r)}</td>
                  </tr>
                );
              })}
            </tbody>
