...
export const Route = createFileRoute("/admin/relatorios")({
  ...
  head: () => ({ meta: [{ title: "Relatórios · De Meo" }] }),
});
            ...
            <tbody>
              {clients.map((c, i) => (
                <tr key={c.id} style={{ background: i % 2 === 0 ? "#fff" : "var(--linen2)", borderTop: "1px solid var(--line)" }}>
                  ...
                  <td className="px-6 py-4 text-[11px]"><span className="dm-badge dm-badge--ok">DFC + DRE</span></td>
                  <td className="px-6 py-4 text-[11px]">
                    <span className="dm-link mr-3">PDF ↓</span>
                    <span className="dm-link">Excel ↓</span>
                  </td>
                </tr>
              ))}
            </tbody>
