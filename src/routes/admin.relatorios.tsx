...
export const Route = createFileRoute("/admin/relatorios")({
  ...
  head: () => ({ meta: [{ title: "Relatórios · Aura" }] }),
});
            ...
            <tbody>
              {clients.map((c, i) => (
                <tr key={c.id} style={{ background: i % 2 === 0 ? "#fff" : "var(--linen2)", borderTop: "1px solid var(--line)" }}>
                  ...
                  <td className="px-6 py-4 text-[11px]"><span className="aura-badge aura-badge--ok">DFC + DRE</span></td>
                  <td className="px-6 py-4 text-[11px]">
                    <span className="aura-link mr-3">PDF ↓</span>
                    <span className="aura-link">Excel ↓</span>
                  </td>
                </tr>
              ))}
            </tbody>
