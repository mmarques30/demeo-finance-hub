-- =============================================================================
-- Módulo 6 — Aurora · CRM (leads, deals, proposals, contracts)
-- Catálogo de serviços + histórico de preços + KPIs de pipeline
-- =============================================================================
-- Pré-requisito: public.profiles(user_id, role) já existe.
-- Storage buckets a criar manualmente no painel Supabase (privados):
--   - proposals
--   - contracts
-- =============================================================================
...
-- Tabela de contadores anuais (para AURORA-YYYY-NNNN com NNNN reiniciando por ano)
...
DECLARE
  ...
  v_n    bigint;
BEGIN
  INSERT INTO public.document_counters (kind, year, next_value)
  VALUES ('proposal', v_year, 1)
...
-- -----------------------------------------------------------------------------
-- 1. Catálogos: lead_sources e deal_stages
-- -----------------------------------------------------------------------------
...
-- Paleta Aurora: green #4A6741, sage #8FA688, navy #1B394D, tan #B8956A, tan2 #D4B896, muted #7A7260
...
-- -----------------------------------------------------------------------------
-- 2. Catálogo de serviços + histórico de preços
-- -----------------------------------------------------------------------------
...
-- Seed inicial do histórico
INSERT INTO public.service_price_history (service_id, price, source, notes)
SELECT id, base_price, 'manual_update', 'Preço inicial (seed)'
...
-- Fecha o ciclo lead <-> deal
ALTER TABLE public.leads
  ADD CONSTRAINT leads_promoted_to_deal_fk
...
CREATE INDEX idx_da_due
  ON public.deal_activities (due_date)
...
CREATE TABLE public.contracts (
  ...
  signature_provider       text     NOT NULL DEFAULT 'manual'
                            CHECK (signature_provider IN ('manual','clicksign')),
...
-- Mudança de stage em deals → grava histórico, atualiza stage_changed_at e closed_at
...
-- Snapshot de preço a cada item de proposta com service_id
...
BEGIN
  IF NEW.number IS NULL OR NEW.number = '' THEN
    NEW.number := public.next_proposal_number();
  END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER fill_proposal_number
...
BEGIN
  IF NEW.number IS NULL OR NEW.number = '' THEN
    NEW.number := public.next_contract_number();
  END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER fill_contract_number
...
CREATE POLICY admin_full_document_counters       ON public.document_counters
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_lead_sources            ON public.lead_sources
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_deal_stages             ON public.deal_stages
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY admin_full_service_price_history   ON public.service_price_history
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_leads                   ON public.leads
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_deals                   ON public.deals
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_deal_stage_history      ON public.deal_stage_history
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_deal_activities         ON public.deal_activities
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_proposals               ON public.proposals
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_proposal_items          ON public.proposal_items
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY admin_full_contracts               ON public.contracts
  FOR ALL TO authenticated
  USING (public.is_admin()) WITH CHECK (public.is_admin());

-- Leitura pública de proposta via header x-proposal-token
CREATE POLICY public_token_read_proposals        ON public.proposals
  FOR SELECT TO anon, authenticated
  USING (
    public_token IS NOT NULL
    AND public_token = (
      current_setting('request.headers', true)::jsonb ->> 'x-proposal-token'
    )
  );

-- Mesma política para os itens (o cliente precisa ler os items para renderizar a proposta)
